...
const Competances = (props) => {
  ...
  const [DataModelCollection, setDataModelCollection] = useState(props.inputDataModel?.competance ? props.inputDataModel?.competance : []);
  ...
  useEffect(() => {
    props.updateModel("competance", DataModelCollection);
  }, [DataModelCollection]);

  useEffect(() => {
    if(typeof props.inputDataModel?.competance != "undefined" ){
      setDataModelCollection(props.inputDataModel["competance"]) 
    }
    ...
    return (
     ...
     <Accordion flush expanded={!Collapsed} onChange={(e,expanded)=>setCollapsed(!expanded)} style={style.bgItem}>
       <AccordionSummary expandIcon={<ExpandMoreIcon />} className="SectionHeading" >
          ...
          <L>Compétences</L> <DashCircle className="remove_section_cione" onClick={(e)=>{e.stopPropagation() ;props.removeSection(props.name);}}  />
        </AccordionSummary>
        {!Collapsed && (
          <AccordionDetails>
            <Row>
