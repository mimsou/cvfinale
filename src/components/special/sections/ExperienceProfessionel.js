...
const ExperienceProfessionel = (props) => {
  const jsonCopy = (src) => {
  ...
  const [DataModelReset, setDataModelReset] = useState({
    ...
    datedebut: { active: true, value: {mois:"",anne:""} },
    datefin: { active: true, value: {mois:"",anne:""} },
  ...
  const [DataModelCollection, setDataModelCollection] = useState( props.inputDataModel?.experienceProfessionel ?  props.inputDataModel?.experienceProfessionel : []);
  ...
  const handleTextareaChange = (values) => {
    const newobj = JSON.parse(JSON.stringify(DataModel));
    newobj["description"].value = values;
    setDataModel(newobj);
  };
  ...
  useEffect(() => {
    props.updateModel("experienceProfessionel", DataModelCollection);
  }, [DataModelCollection]);

  useEffect(() => {
    if(typeof props.inputDataModel?.experienceProfessionel != "undefined" ){
      setDataModelCollection(props.inputDataModel["experienceProfessionel"]) 
    }
    ...
    return (
      ...
      <Accordion flush expanded={!Collapsed} onChange={(e,expanded)=>setCollapsed(!expanded)} style={style.bgItem}>
        ...
        <AccordionSummary expandIcon={<ExpandMoreIcon />} className="SectionHeading" >
           ...
           <L>Expériences</L>  <DashCircle className="remove_section_cione" onClick={(e)=>{e.stopPropagation() ;props.removeSection(props.name);}}  />
...
export default ExperienceProfessionel;
